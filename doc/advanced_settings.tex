...
\begin{table}
	...
	\begin{tabular}[t]{| p{.15\textwidth} | p{.15\textwidth} | p{.65\textwidth} |}
			...
			\tablinethree{Use Interview Limits}{True / False}
							{Apply limits to the minimum and maximum number of interviews that can be assigned to a person.  These limits are specified in \texttt{Settings} in the \texttt{Minimum Number of Interviews} and \texttt{Maximum Number of Interviews} properties}		
			\tablinethree{Choice Exponent}{0 - 10}
							{A factor affecting how much first preferences are weighted over last preferences.  For more information, see note on Weights, below.}		
